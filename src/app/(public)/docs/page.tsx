      ...
      <div className="container max-w-7xl relative z-10 px-6">
        <header className="max-w-4xl mb-24 animate-slide-up">
          ...
          <h1 className="text-[clamp(2.5rem,8vw,5rem)] font-bold tracking-tighter leading-[0.9] text-white mb-12">
            Centre de <br />
            <span className="text-white/30">connaissances.</span>
          </h1>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 reveal">
          {sections.map((section, i) => (
            <div
              key={i}
              className="glass-card p-12 rounded-xl border-border group hover:border-accent/20 transition-all"
            >
              ...
              <ul className="space-y-4">
                {section.items.map((item, j) => (
                  <li key={j}>
                    ...
                    >
                      <div className="w-1 h-4 bg-white/10 mr-4 group-hover/link:bg-accent transition-colors" />
                      {item}
                    </Link>
                  </li>
                ))}
        ...
        <div className="mt-40 p-16 glass-card rounded-xl text-center border-accent/10 reveal">
          <h2 className="text-4xl font-bold text-white mb-8 tracking-tight">
            Besoin d&apos;aide{" "}
            <span className="text-white/40">personnalisée ?</span>
          </h2>
          <button className="px-12 py-5 rounded-full bg-accent text-background font-black text-lg hover:bg-accent-bright transition-all">
